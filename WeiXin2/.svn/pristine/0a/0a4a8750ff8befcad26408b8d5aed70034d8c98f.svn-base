...
Page({
  ...
  onLoad: function (options) {
    ...
    var address0;
    wx.request({
      url: url+'findUserById?userId=' + options.V_userid,
      data: {
      },
      header: {
      ...
      success: function (res) {
        ...
        nickname0 = res.data.userName;
        male0 = res.data.userSex;
        birthday0 = res.data.birthday.substring(0,10);
        numberPlate0 = res.data.cardNumber;
        address0 = res.data.province;
      },
      complete: function () {
        if (userImage0 == null) {
